...
int main()
{
    ...
    cout << "Quick sort with recursion:" << endl << endl;
    ...
    for (int i = 0; i < k; i++)
        testArray[i] = rand() % 100001 - 50000;

   // замеряем начало работы и конец работы функции Search
    chrono::steady_clock::time_point start = chrono::steady_clock::now();
    Search(testArray, k, testArray[648]);
    chrono::steady_clock::time_point end = chrono::steady_clock::now();

    // выводим время работы как разность начала и конца
    cout << "Search time: " << chrono::duration_cast<chrono::nanoseconds>(end - start).count() << " ns" << endl;

    // сортируем массив
    QuickSort(testArray, 0, k-1);

    // замеряем начало работы и конец работы функции BSearch
    start = chrono::steady_clock::now();
    BSearch(testArray[648], testArray, 0, k);
    end = chrono::steady_clock::now();

    // выводим время работы как разность начала и конца
    cout << "Bsearch time: " << chrono::duration_cast<chrono::nanoseconds>(end - start).count() << " ns" << endl;
}
