 ...
 *  - Внутри класса PlayField определить и использовать public класс положения клетки CellPos (CellIdx),
 * хранящий координаты точки в виде двух значений: строка, столбец. Определить для класса CellIdx соответствующие
 * конструкторы, сеттеры (если нужно), геттеры, по возможности проверять правильность задаваемых данных.
 * Строки и столбцы нумеруются в диапазоне [0, 2]. Для удобства применения можно определить для CellPos оператор
 * явного или неявного приведения к int, чтобы использовать напрямую при обращении к линейному массиву ячеек.
 ...
 *
 * ------- Ссылки в помощь -------
 * https://ru.cppreference.com/w/cpp/utility/initializer_list ///хорошой для инициализации CellPos (CellIdx)
 * https://ru.cppreference.com/w/cpp/error/assert
*/
